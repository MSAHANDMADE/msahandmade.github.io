...
(function () {
  ...
  // EmailJS (verifică să fie fix acestea din contul tău)
  const EMAILJS_PUBLIC_KEY = "iSadfb7-TV_89l_6k";
  const EMAILJS_SERVICE_ID = "service_ix0zpp7";
  const TEMPLATE_CLIENT_ID = "template_9yctwor"; // Order Confirmation
  const TEMPLATE_ADMIN_ID  = "template_13qpqtt"; // Contact Us -> New Order
  ...
  function bindRowEvents(){
    $all(".qminus").forEach(btn=>btn.addEventListener("click",()=>{
      const tr=btn.closest("tr"); const id=tr.dataset.id; const input=$(".qinput",tr);
      const next=Math.max(0,(+input.value||0)-1); input.value=next; setQty(id,next); rerenderRow(tr);
    }));
    ...
    $all(".qinput").forEach(inp=>inp.addEventListener("change",()=>{
      const tr=inp.closest("tr"); const id=tr.dataset.id; const val=Math.max(0,+inp.value||0);
      inp.value=val; setQty(id,val); rerenderRow(tr);
    }));
    $all(".qdel").forEach(btn=>btn.addEventListener("click",()=>{
      const tr=btn.closest("tr"); const id=tr.dataset.id; removeFromCart(id); tr.remove();
  ...
  function buildProformaHTML(cart,t,data){
    const rows=cart.map(i=>`
      <tr>
        ...
        <td style="text-align:right">${money(i.price)}</td><td style="text-align:right">${money(i.price*i.qty)}</td>
      </tr>`).join('');
    return `
    ...
    form.addEventListener("submit", async (e)=>{
      ...
      try{
        if(window.emailjs){
          ...
          console.warn("EmailJS indisponibil în pagină.");
        }
      } catch(err){
