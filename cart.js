/* MSA Handmade – cart.js (stable, with proforma & EmailJS) */
/* Stocare localStorage + randare coș + reduceri + livrare + email */

(function () {
  const LS_KEY = "msa_cart_v1";

  // ---------- EmailJS (completează din contul tău) ----------
  const EMAILJS_PUBLIC_KEY = "iSadfb7-TV_89l_6k";            // Public Key
  const EMAILJS_SERVICE_ID = "service_ix0zpp7";              // Service ID (Gmail)
  const TEMPLATE_CLIENT_ID = "template_9yctwor";             // "Order Confirmation" (are {{html_proforma}})
  const TEMPLATE_ADMIN_ID  = "template_13qpqtt";             // "Contact Us" – folosit ca "New Order" la tine

  // ---------- Reguli ----------
  const rules = {
    shippingFlat: 17,
    freeShipFrom: 300, // livrare 0 dacă SUBTOTAL >= 300 (indiferent de reducere)
    discounts: [
      { min: 400, pct: 0.20 },
      { min: 300, pct: 0.15 },
      { min: 200, pct: 0.10 },
  ...
  function totals(cart) {
    ...
    // LIVRARE 0 dacă SUBTOTAL >= freeShipFrom, indiferent de discount
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        if (window.emailjs) {
          ...
          // 1) către CLIENT (Order Confirmation)
          await emailjs.send(EMAILJS_SERVICE_ID, TEMPLATE_CLIENT_ID, {
            to_email: data.email,           // IMPORTANT pentru „To Email: {{to_email}}”
            nume: data.nume || '',
          ...
          // 2) către ADMIN (nouă comandă)
          await emailjs.send(EMAILJS_SERVICE_ID, TEMPLATE_ADMIN_ID, {
        ...
        console.warn("Eroare EmailJS:", err);
        // continuăm oricum către pagina de mulțumire
      }

      // curățăm coșul și mergem la pagina de „Mulțumim”
      clearCart();
